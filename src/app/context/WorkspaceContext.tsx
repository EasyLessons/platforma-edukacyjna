...
export function WorkspaceProvider({ children }: { children: ReactNode }) {
  ...
  const createWorkspace = async (data: WorkspaceCreate): Promise<Workspace> => {
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Nie udało się utworzyć workspace'a";
      throw new Error(errorMessage);
    }
  };
  ...
  const updateWorkspace = async (id: number, data: WorkspaceUpdate): Promise<Workspace> => {
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Nie udało się zaktualizować workspace'a";
      throw new Error(errorMessage);
    }
  };
  ...
  const toggleFavourite = async (id: number, isFavourite: boolean): Promise<void> => {
    try {
      ...
      setWorkspaces((prev: Workspace[]) =>
        prev.map((ws: Workspace) => (ws.id === id ? { ...ws, is_favourite: isFavourite } : ws))
      );
