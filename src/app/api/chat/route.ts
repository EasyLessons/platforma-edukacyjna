...
function checkRateLimit(ip: string): boolean {
  ...
  userLog.timestamps = userLog.timestamps.filter((time) => now - time < CONFIG.RATE_LIMIT_WINDOW);
...
setInterval(
  () => {
    ...
    for (const [ip, log] of requestLog.entries()) {
      if (log.timestamps.length === 0 && (!log.blockUntil || now >= log.blockUntil)) {
  ...
  try {
    ...
    try {
      ...
      if (isQuotaError && CONFIG.GEMINI_MODEL_FALLBACK) {
        console.log(
          `⚠️ Primary model (${CONFIG.GEMINI_MODEL}) quota exceeded, trying fallback: ${CONFIG.GEMINI_MODEL_FALLBACK}`
        );
    ...
    if (error.message?.includes('API_KEY')) {
      return NextResponse.json(
        {
          response: 'Błąd konfiguracji API. Skontaktuj się z administratorem.',
          error: 'api_key_error',
        },
        { status: 500 }
      );
    ...
    if (error.message?.includes('quota') || error.message?.includes('limit')) {
      return NextResponse.json(
        {
          response: 'Przekroczono limit API. Spróbuj ponownie za chwilę.',
