...
export function PenTool({
  ...
  // ✅ BEZPOŚREDNIA AKTUALIZACJA SVG (bez React!)
  const updatePath = () => {
  ...
  useEffect(() => {
    ...
    const handleDown = (e: PointerEvent) => {
      gestures.handlePointerDown(e as any);
      if (gestures.isGestureActive()) return;
    ...
    const handleMove = (e: PointerEvent) => {
      ...
      pointsRef.current.push(worldPoint);
      updatePath(); // ← BEZPOŚREDNIA AKTUALIZACJA!
    };
    ...
    return () => {
      ...
      overlay.removeEventListener('pointercancel', handleCancel);
    };
  }, [viewport, canvasWidth, canvasHeight, color, lineWidth, onPathCreate, gestures]);
    ...
    <div className="absolute inset-0 z-20" style={{ cursor: 'crosshair' }}>
      ...
      >
        <path
          ref={pathRef}
          stroke={color}
          strokeWidth={clampLineWidth(lineWidth, viewport.scale)}
          strokeLinecap="round"
          strokeLinejoin="round"
          fill="none"
        />
