...
export function TextTool({
  ...
  const handleOverlayDown = (e: React.MouseEvent) => {
    ...
    const rect = overlayRef.current?.getBoundingClientRect() ?? { left: 0, top: 0 };
    const world = inverseTransformPoint(
      { x: e.clientX - rect.left, y: e.clientY - rect.top },
      viewport,
      canvasWidth,
      canvasHeight
  ...
  const handleOverlayUp = () => {
    ...
    const sx = drawStartWorld.current.x,
      sy = drawStartWorld.current.y;
    const ex = drawCurrWorld.current.x,
      ey = drawCurrWorld.current.y;
  ...
  if (draft && phase === 'editing') {
    ...
    const br = transformPoint(
      { x: draft.worldX + draft.worldW, y: draft.worldY + localHeight },
      viewport,
      canvasWidth,
      canvasHeight
    );
  ...
  if (phase === 'drawing' && drawPreview) {
    ...
    const br = transformPoint(
      { x: drawPreview.x + drawPreview.w, y: drawPreview.y + drawPreview.h },
      viewport,
      canvasWidth,
      canvasHeight
    );
  ...
  return (
    <div
      ...
      style={{ cursor: phase === 'editing' ? 'default' : 'text' }}
    >
      {phase !== 'editing' && (
        <div
          ref={overlayRef}
      ...
      {phase === 'editing' && draft && (
        <div
          ref={(node) => {
            ...
            if (editorDivRef)
              (editorDivRef as React.MutableRefObject<HTMLDivElement | null>).current = node;
          }}
        ...
        >
          ...
          <div
            ...
            onMouseDown={makeResizeHandler('tl')}
          />
          <div
            style={{
              ...handleStyle,
            ...
            onMouseDown={makeResizeHandler('tr')}
          />
          <div
            style={{
              ...handleStyle,
            ...
            onMouseDown={makeResizeHandler('bl')}
          />
          <div
            style={{
              ...handleStyle,
            ...
            onMouseDown={makeResizeHandler('br')}
          />
        </div>
      )}
    </div>
