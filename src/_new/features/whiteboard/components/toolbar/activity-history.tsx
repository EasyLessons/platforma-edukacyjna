 ...
 * Grupuje elementy po autorze i czasie (5s okno)
 */
const groupElementsByAuthorAndTime = (
  elements: BoardElementWithAuthor[],
  ...
  const sorted = [...elements]
    .filter((el) => el.created_at) // Wystarczy sam czas
    .sort((a, b) => {
          ...
          <div className="flex items-center gap-2 flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs text-gray-400">{formatRelativeTime(group.timestamp)}</span>
              </div>
...
const ActivityHistoryComponent = ({
  ...
  const groups = useMemo(() => {
    return groupElementsByAuthorAndTime(elements, user);
  }, [elements, user]);
  ...
  const handleShow = useCallback(
    ...
    [onCenterView, onSelectElements, viewport.scale, onClose]
  );

  return (
    <div className="absolute top-20 bottom-30 right-4 z-50 pointer-events-auto flex flex-col w-72 bg-white rounded-lg shadow-xl border border-gray-200 overflow-hidden">
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto min-h-0 bg-white">
        {groups.length === 0 ? (
          <div className="px-4 py-12 text-center text-gray-400 flex flex-col items-center">
            ...
            <p className="text-sm font-medium">Brak historii</p>
            <p className="text-xs mt-1">Aktywność pojawi się po dodaniu elementów na tablicę.</p>
          </div>
        ) : (
