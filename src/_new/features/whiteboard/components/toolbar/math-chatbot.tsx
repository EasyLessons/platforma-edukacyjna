...
function MathChatbotInner({
  ...
  // Telefon: babelek Tutora jako okragla ikona 44 px zamiast pigulki z napisem.
  const iconOnlyBubble = uiMetrics.isPhoneLayout;
  ...
  if (isCollapsed && !isExiting) {
    return (
      <div
        ...
        style={{
          right: safeInset(20, 'right'),
          bottom: safeInset(20, 'bottom'),
      ...
      >
        <button
          ...
          aria-label="Tutor AI"
          className={`cursor-pointer font-semibold hover-shine flex items-center justify-center gap-2 ${iconOnlyBubble ? 'h-11 w-11 px-0' : 'h-10 px-4'} rounded-full bg-gray-200 hover:bg-gray-300 text-gray-700 transition-all duration-300 ease-in-out shadow-md group`}
        ...
        >
          ...
          {!iconOnlyBubble && <span className="text-[13px]">Tutor AI</span>}
        </button>
      </div>
    );
  }
    ...
    <>
      ...
      <div
        ...
        style={{
          ...
          bottom: safeInset(10, 'bottom'),
          right: safeInset(10, 'right'),
          paddingTop: '120px',
          animation: isExiting
