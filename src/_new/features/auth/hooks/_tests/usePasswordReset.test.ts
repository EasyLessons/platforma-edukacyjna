...
describe('usePasswordReset', () => {
  ...
  describe('krok 1 — email', () => {
    it('ustawia błąd dla nieprawidłowego emaila', async () => {
      const { result } = setup();
      act(() => {
        result.current.handleEmailChange('zly');
      });
      await act(async () => {
        await result.current.handleSendEmail(fakeEvent);
      });
    ...
    it('przechodzi do kroku code po sukcesie', async () => {
      const { result } = setup();
      act(() => {
        result.current.handleEmailChange('jan@example.com');
      });
      await act(async () => {
        await result.current.handleSendEmail(fakeEvent);
      });
    ...
    it('ustawia emailError przy błędzie API', async () => {
      vi.mocked(requestPasswordReset).mockRejectedValue(
        new AppError('Nie znaleziono', 'NOT_FOUND', 404)
      );
      const { result } = setup();
      act(() => {
        result.current.handleEmailChange('jan@example.com');
  ...
  describe('krok 2 — kod', () => {
    it('przechodzi do kroku password po weryfikacji kodu', async () => {
      ...
      await act(async () => {
        await result.current.handleCodeVerified('123456');
      });
    ...
    it('rzuca gdy kod nieprawidłowy — useCodeInput to obsługuje', async () => {
      vi.mocked(verifyResetCode).mockRejectedValue(
        new AppError('Zły kod', 'VALIDATION_ERROR', 422)
      );
      const { result } = setup();
      await expect(
        act(async () => {
          await result.current.handleCodeVerified('000000');
        })
      ).rejects.toThrow();
    });
    ...
    it('handleCodeResend ponownie wysyła email', async () => {
      ...
      await act(async () => {
        await result.current.handleCodeResend();
      });
