...
Główne funkcje:
  create_notification()       — tworzy rekord w bazie (wywoływana wewnętrznie,
                                 np. z create_invite w workspace service)
...
def create_notification(
    ...
    """
    Tworzy powiadomienie w bazie.
 
    Wywoływana wewnętrznie — nie jest publicznym endpointem.
    Przykład użycia w workspace service po wysłaniu zaproszenia:
 
        from dashboard.notifications.service import create_notification
...
def get_user_notifications(
    ...
    `limit` domyślnie 50 — wystarczy dla panelu powiadomień.
    Przy budowaniu paginacji można dodać `offset` parametr.
    """
    notifications = (
