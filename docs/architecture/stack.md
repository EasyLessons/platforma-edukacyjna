...
**Supabase (Realtime + Storage, NIE Auth!)** — Broadcast + Presence po stronie frontendu (client: `src/_new/lib/supabase/client.ts`): powiadomienia, kursory/presence/typing na tablicy, sygnalizacja voice chatu i — w ścieżce legacy — synchronizacja elementów tablicy. Storage po stronie backendu (bucket `board-images`, `backend/api/v1/whiteboard/storage.py`) na obrazy wklejane na tablicę. Auth jest własne (patrz wyżej).
