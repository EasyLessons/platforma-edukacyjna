...
- **User** — konto użytkownika. Obsługuje zarówno logowanie hasłem, jak i Google OAuth (`hashed_password` nullable, `google_id`, `auth_provider`). Ma `active_workspace_id` — wskazuje, w którym workspace użytkownik "aktualnie siedzi" w UI.
