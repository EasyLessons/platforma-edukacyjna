...
\begin{itemize}[leftmargin=*]
    ...
    \item Rozmowy głosowe w ramach tablicy
\end{itemize}
...
\subsection{Model danych (schemat bazy)}

Baza danych PostgreSQL (hostowana na Neon) zawiera 10 tabel. Poniżej opis każdej encji wraz z kluczowymi polami i relacjami.

\begin{itemize}[leftmargin=*]
    \item \textbf{users} -- konta użytkowników; pola: \texttt{id}, \texttt{username}, \texttt{email}, \texttt{hashed\_password} (bcrypt), \texttt{google\_id}, \texttt{auth\_provider} (,,email'' lub ,,google''), \texttt{avatar\_url}, \texttt{active\_workspace\_id}
    \item \textbf{workspaces} -- przestrzenie robocze; pola: \texttt{id}, \texttt{name}, \texttt{icon}, \texttt{bg\_color}, \texttt{created\_by} (FK→users)
    \item \textbf{workspace\_members} -- relacja użytkownik$\leftrightarrow$workspace; pola: \texttt{workspace\_id}, \texttt{user\_id}, \texttt{role} (editor/admin), \texttt{is\_favourite}
    \item \textbf{boards} -- tablice interaktywne; pola: \texttt{id}, \texttt{workspace\_id}, \texttt{name}, \texttt{icon}, \texttt{bg\_color}, \texttt{settings} (JSONB), \texttt{last\_modified}, \texttt{last\_modified\_by}
    \item \textbf{board\_users} -- obecność i dostęp użytkownika do tablicy; pola: \texttt{board\_id}, \texttt{user\_id}, \texttt{is\_online}, \texttt{is\_favourite}, \texttt{last\_opened}
    \item \textbf{board\_elements} -- elementy rysunkowe na tablicy; pola: \texttt{board\_id}, \texttt{element\_id} (UUID z frontendu), \texttt{type} (path/rect/text/image/\ldots), \texttt{data} (\textbf{JSONB} -- współrzędne, kolor, grubość itp.), \texttt{is\_deleted} (miękkie usunięcie dla undo/redo)
    \item \textbf{workspace\_invites} -- zaproszenia do workspace'a; pola: \texttt{workspace\_id}, \texttt{invited\_by}, \texttt{invited\_id}, \texttt{invite\_token}, \texttt{expires\_at}, \texttt{is\_used}
    \item \textbf{notifications} -- powiadomienia użytkownika; pola: \texttt{user\_id}, \texttt{type}, \texttt{payload} (JSONB), \texttt{is\_read}; indeks częściowy na nieprzeczytanych powiadomieniach
    \item \textbf{refresh\_tokens} -- refresh tokeny sesji; pola: \texttt{user\_id}, \texttt{token\_hash} (SHA-256), \texttt{expires\_at} (7 dni), \texttt{revoked}
    \item \textbf{saved\_assets} -- szablony elementów zapisane przez użytkownika; pola: \texttt{user\_id}, \texttt{name}, \texttt{elements\_data} (JSONB), \texttt{thumbnail} (base64)
\end{itemize}

\textbf{Kluczowe relacje:}
\begin{itemize}[leftmargin=*]
    \item \texttt{users} 1:N \texttt{workspaces} (creator), 1:N \texttt{workspace\_members}, 1:N \texttt{notifications}, 1:N \texttt{saved\_assets}
    \item \texttt{workspaces} 1:N \texttt{boards}, 1:N \texttt{workspace\_members}, 1:N \texttt{workspace\_invites}
    \item \texttt{boards} 1:N \texttt{board\_elements}, 1:N \texttt{board\_users}
\end{itemize}

\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=\faInfoCircle\ JSONB w board\_elements]
Kolumna \texttt{data} w tabeli \texttt{board\_elements} używa typu \textbf{JSONB} (PostgreSQL). Przechowuje pełny stan elementu rysunkowego (punkty ścieżki, kolor, grubość linii, tekst itp.) bez stałego schematu -- co umożliwia łatwe dodawanie nowych typów elementów bez migracji bazy.
\end{tcolorbox}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Metoda:} JWT (JSON Web Tokens) -- dwutokenowy model sesji
    \item \textbf{Access token:} Czas życia \textbf{15 minut}; przechowywany po stronie klienta; używany do autoryzacji żądań API
    \item \textbf{Refresh token:} Czas życia \textbf{7 dni}; ustawiany jako \textbf{HttpOnly cookie} (niedostępny z JavaScript -- ochrona przed kradzieżą przez XSS); rotowany przy każdym odświeżeniu sesji
    \item \textbf{Odświeżanie sesji:} Po wygaśnięciu access tokena frontend automatycznie wywołuje endpoint \texttt{/auth/refresh}, który wydaje nowy access token i nowy refresh token (rotacja)
    \item \textbf{Wylogowanie:} Unieważnia refresh token w bazie i kasuje cookie
...
\begin{enumerate}
    ...
    \item \textbf{Strzałka (Arrow)} -- rysowanie strzałek kierunkowych na tablicy
\end{enumerate}

\begin{tcolorbox}[colback=green!5!white,colframe=green!75!black,title=\faLightbulb\ Pro Tip]
Większość narzędzi posiada przypisane skróty klawiszowe (pojedyncze litery) dla szybszego dostępu. Wyjątkiem jest narzędzie \textbf{Strzałka}, które nie posiada skrótu ani przycisku w pasku narzędzi. Pełna lista skrótów znajduje się w rozdziale~9.
\end{tcolorbox}
...
\begin{itemize}
    \item Typ kształtu: Prostokąt, Koło, Trójkąt, Linia, Wielokąt
...
\hline
\multicolumn{2}{|l|}{\textit{Strzałka (Arrow) -- brak skrótu; narzędzie dostępne bez przycisku w pasku}} \\
\hline
\end{tabular}
...
Skróty narzędzi to pojedyncze litery (bez Ctrl/Cmd). Narzędzie \textbf{Strzałka} nie posiada skrótu klawiszowego ani przycisku w pasku -- jest dostępne wyłącznie programowo.
\end{tcolorbox}
...
\subsection{Uruchomienie przez Docker (zalecane)}

Najprostszy sposób uruchomienia całego środowiska -- frontend, backend i konfiguracja sieciowa -- za pomocą jednej komendy.
...
# Uzupelnij wartosci w obu plikach (patrz sekcja o zmiennych srodowiskowych)
...
\begin{itemize}
    ...
    \item Dokumentacja API (Swagger): \url{http://localhost:8000/docs}
\end{itemize}
...
\begin{tcolorbox}[colback=green!5!white,colframe=green!75!black,title=\faLightbulb\ Wskazówka]
Docker automatycznie instaluje zależności i konfiguruje środowisko. Jest to zalecana metoda dla nowych deweloperów oraz do szybkiego uruchomienia projektu bez konfigurowania lokalnego środowiska Python/Node.
\end{tcolorbox}

\subsection{Uruchomienie ręczne}

Alternatywna metoda dla deweloperów preferujących bezpośrednią pracę z kodem.
...
\begin{itemize}
    \item \textbf{Node.js} (wersja 20+) -- do uruchomienia frontendu
    \item \textbf{npm} -- menedżer pakietów JavaScript
    \item \textbf{Python} (wersja 3.12+) -- do uruchomienia backendu
    \item \textbf{pip} -- menedżer pakietów Python
    \item \textbf{Git} -- system kontroli wersji
\end{itemize}

\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=\faInfoCircle\ Baza danych]
Baza danych PostgreSQL działa w chmurze (\textbf{Neon} -- serverless PostgreSQL). \textbf{Nie jest wymagana lokalna instalacja PostgreSQL.} Connection string do bazy konfiguruje się przez zmienną środowiskową \texttt{DATABASE\_URL} w pliku \texttt{backend/.env}.
\end{tcolorbox}
...
Projekt wymaga dwóch plików ze zmiennymi środowiskowymi: \texttt{.env.local} (frontend) oraz \texttt{backend/.env} (backend).

\subsubsection{Zmienne backendu (\texttt{backend/.env})}

\begin{lstlisting}[language=bash]
# Baza danych (Neon -- connection string z panelu Neon.tech)
...
# Tokeny sesji
COOKIE_SECURE=false          # true w produkcji (HTTPS)
COOKIE_DOMAIN=               # np. .easylesson.app w produkcji

# Email (Resend)
RESEND_API_KEY=re_xxxxxxxxxxxx
FROM_EMAIL=noreply@twojadomena.pl
...
\subsubsection{Zmienne frontendu (\texttt{.env.local})}

\begin{lstlisting}[language=bash]
# Backend API
NEXT_PUBLIC_API_URL=http://localhost:8000

# Supabase (publiczne klucze)
...
# WebRTC -- voice chat (Xirsys)
NEXT_PUBLIC_XIRSYS_IDENT=your-xirsys-ident
NEXT_PUBLIC_XIRSYS_SECRET=your-xirsys-secret
NEXT_PUBLIC_XIRSYS_CHANNEL=your-xirsys-channel
\end{lstlisting}

\textbf{Ważne:} Zastąp wartości placeholder swoimi własnymi. Nigdy nie commituj plików \texttt{.env} do repozytorium!
...
\subsubsection{Błąd: \texttt{DATABASE CONNECTION FAILED} (połączenie z Neon)}

Sprawdź:
\begin{enumerate}
    \item Czy \texttt{DATABASE\_URL} w \texttt{backend/.env} zawiera poprawny connection string z panelu Neon.tech
    \item Czy string zawiera parametr \texttt{sslmode=require}
    \item Czy skopiowałeś cały string (nie obcięty)
\end{enumerate}
...
platforma-edukacyjna/
  backend/                    # API - FastAPI + SQLAlchemy
    ...
    Dockerfile                # Kontener backendu
    alembic/                  # Migracje bazy danych
    core/                     # Config, models, database, logging
    api/
      v1/
        auth/                 # Logowanie, rejestracja, OAuth
        boards/               # Zarzadzanie tablicami
        workspaces/           # Zarzadzanie workspace'ami
        whiteboard/           # Elementy tablicy (API)
        assets/               # Zapisane szablony
        notifications/        # Powiadomienia
  src/                        # Frontend - Next.js + React + TypeScript
    app/                      # Routing (Next.js App Router)
    _new/
      features/
        whiteboard/           # Tablica interaktywna (glowna funkcja)
          commands/           # Command pattern (undo/redo)
          engine/             # WhiteboardEngine (fasada)
          tools/              # Rejestr narzedzi (11 narzedzi)
          stores/             # Zustand -- stan globalny
          hooks/              # Hooki (elementy, historia, viewport)
          components/         # Komponenty UI tablicy
        auth/                 # Autentykacja (frontend)
        board/                # Zarzadzanie tablicami (frontend)
        workspace/            # Zarzadzanie workspace'ami (frontend)
        notifications/        # Powiadomienia (frontend)
      shared/                 # Wspoldzielone komponenty i narzedzia
  public/                     # Zasoby statyczne
  Dockerfile                  # Kontener frontendu
  docker-compose.yml          # Orchestracja kontenerow
  .env.local                  # Zmienne srodowiskowe frontend (nie commitowac!)
...
alembic upgrade head                 # Uruchomienie migracji
uvicorn main:app --reload            # Start (tryb deweloperski)

# Frontend
npm install                          # Instalacja zaleznosci
npm run dev                          # Start (tryb deweloperski)
npm run build                        # Build produkcyjny
npm start                            # Start (tryb produkcyjny)

# Docker (cale srodowisko)
docker compose up --build            # Buduj i uruchom
docker compose down                  # Zatrzymaj kontenery
docker compose logs -f               # Podglad logow
\end{lstlisting}
...
\newpage
\section{Dokumentacja testów}

\subsection{Cel i zakres testowania}

Celem procesu testowania jest weryfikacja poprawności działania systemu EasyLesson na poziomie logiki biznesowej oraz warstwy komunikacji HTTP. Testy obejmują moduły autentykacji, zarządzania przestrzeniami roboczymi i tablicami, powiadomień oraz tras API obsługujących asystenta AI i formularz kontaktowy.

\subsection{Strategia testowania}

W projekcie zastosowano dwa poziomy testowania:

\begin{center}
\begin{tabular}{lll}
\toprule
\textbf{Poziom} & Jednostkowy & Integracyjny \\
\midrule
\textbf{Typ} & white-box & black-box HTTP \\
\textbf{Narzędzie} & pytest / Vitest & FastAPI TestClient / Vitest node \\
\textbf{Zakres} & serwisy, hooki, narzędzia pomocnicze & routery REST, trasy Next.js API \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Wzorzec testów backendowych.} Każdy test integracyjny używa bazy SQLite in-memory z pulą \texttt{StaticPool}, wstrzykiwanej przez mechanizm \texttt{dependency\_overrides[get\_db]}. Token JWT tworzony jest bezpośrednio funkcją \texttt{create\_access\_token} bez przechodzenia przez endpoint logowania. Gwarantuje to izolację i powtarzalność każdego przypadku.

\textbf{Wzorzec testów frontendowych.} Trasy Next.js API testowane są w środowisku \texttt{node} (dyrektywa \texttt{// @vitest-environment node}). Zewnętrzne zależności - SDK Gemini oraz Resend API - są mockowane przez \texttt{vi.mock}, co eliminuje opłaty i niedeterminizm sieci.
...
\begin{tabular}{lll}
\toprule
\textbf{Warstwa} & \textbf{Narzędzie} & \textbf{Wersja} \\
\midrule
Backend & Python & 3.13.2 \\
Backend & pytest & 8.4.2 \\
Backend & FastAPI TestClient (Starlette) & 0.115.x \\
Backend & SQLAlchemy / SQLite in-memory & 2.x \\
Frontend & Node.js & 20.x \\
Frontend & Vitest & 3.x \\
Frontend & @testing-library/react & 16.x \\
\bottomrule
\end{tabular}
\end{center}
...
Poniższa tabela zawiera reprezentatywne przypadki testowe obejmujące wszystkie główne moduły systemu. Kolumna \emph{Typ} wskazuje poziom testowania: \textbf{J} - jednostkowy, \textbf{I} - integracyjny (HTTP).

\begingroup
\scriptsize
\setlength{\tabcolsep}{2pt}
\begin{longtable}{p{1cm} p{3cm} p{1.4cm} p{0.7cm} p{4.5cm} p{4.5cm}}
\toprule
\textbf{ID} & \textbf{Nazwa} & \textbf{Moduł} & \textbf{Typ} & \textbf{Dane wejściowe / warunki} & \textbf{Oczekiwany wynik} \\
\midrule
\endfirsthead
\toprule
\textbf{ID} & \textbf{Nazwa} & \textbf{Moduł} & \textbf{Typ} & \textbf{Dane wejściowe / warunki} & \textbf{Oczekiwany wynik} \\
...
\endhead
\midrule
\multicolumn{6}{r}{\footnotesize Kontynuacja na następnej stronie\ldots} \\
\endfoot
\bottomrule
\endlastfoot

TC-01 & Rejestracja - poprawne dane & Auth & I &
  \texttt{POST /register}: username, email, hasło &
  201; użytkownik nieaktywny (\texttt{is\_active=False}); kod weryfikacyjny w~DB \faCheck \\
\addlinespace

TC-02 & Rejestracja - duplikat e-maila & Auth & I &
  \texttt{POST /register}: e-mail już istnieje w~DB &
  409 Conflict; użytkownik nie zostaje utworzony \faCheck \\
\addlinespace

TC-03 & Logowanie - poprawne dane & Auth & I &
  \texttt{POST /login}: prawidłowy username + hasło &
  200; odpowiedź zawiera \texttt{access\_token} w~formacie JWT \faCheck \\
\addlinespace

TC-04 & Weryfikacja e-mail - poprawny kod & Auth & I &
  \texttt{POST /verify-email}: właściwy kod dla istniejącego konta &
  200; \texttt{is\_active=True} w~DB; kod usunięty z~rekordu \faCheck \\
\addlinespace

TC-05 & Rotacja tokenu odświeżającego & Auth & J &
  \texttt{refresh\_token} ważny, zapisany w~DB &
  Nowy \texttt{access\_token}; stary refresh token unieważniony; nowy zapisany \faCheck \\
\addlinespace

\midrule

TC-06 & Lista przestrzeni - izolacja użytkowników & Workspace & I &
  \texttt{GET /workspaces}; dwa konta z~oddzielnymi WS &
  Odpowiedź zawiera wyłącznie WS zalogowanego usera \faCheck \\
\addlinespace

TC-07 & Tworzenie przestrzeni roboczej & Workspace & I &
  \texttt{POST /workspaces}: name, icon, bg\_color &
  201; \texttt{is\_owner=True}; rekord WorkspaceMember z~rolą \emph{owner} w~DB \faCheck \\
\addlinespace

TC-08 & Edycja WS przez nie-ownera & Workspace & I &
  \texttt{PUT /workspaces/\{id\}}: token użytkownika z~rolą \emph{editor} &
  403 Forbidden \faCheck \\
\addlinespace

TC-09 & Usunięcie członka przez ownera & Workspace & I &
  \texttt{DELETE /workspaces/\{id\}/}\newline\texttt{members/\{uid\}}: token ownera &
  200; brak rekordu WorkspaceMember w~DB \faCheck \\
\addlinespace

TC-10 & Akceptacja wygasłego zaproszenia & Workspace & I &
  \texttt{POST /invites/accept/\{token\}}: pole \texttt{expires\_at} w~przeszłości &
  400 / 410; użytkownik nie zostaje dodany do~WS \faCheck \\
\addlinespace

\midrule

TC-11 & Lista tablic workspace & Board & I &
  \texttt{GET /boards?workspace\_id=X}: token właściciela WS &
  200; tablica testowa widoczna w~odpowiedzi \faCheck \\
\addlinespace

TC-12 & Tworzenie tablicy - brak nazwy & Board & I &
  \texttt{POST /boards}: brak pola \texttt{name} &
  422 Unprocessable Entity \faCheck \\
\addlinespace

TC-13 & Aktualizacja tablicy & Board & I &
  \texttt{PUT /boards/\{id\}}: nowa nazwa \emph{Zmieniona nazwa} &
  200; \texttt{data.name == "Zmieniona nazwa"} \faCheck \\
\addlinespace

TC-14 & Przełączenie flagi \emph{ulubione} & Board & I &
  \texttt{POST /boards/\{id\}/}\newline\texttt{toggle-favourite}: \texttt{\{is\_favourite: true\}} &
  200; \texttt{data.is\_favourite == true} \faCheck \\
\addlinespace

\midrule

TC-15 & Pusta lista powiadomień & Notification & I &
  \texttt{GET /notifications}: brak rekordów dla usera &
  200; \texttt{data.notifications == []} \faCheck \\
\addlinespace

TC-16 & Oznaczenie powiadomienia jako przeczytane & Notification & I &
  \texttt{PATCH /notifications/\{id\}/read} &
  200; \texttt{is\_read=True} w~DB \faCheck \\
\addlinespace

TC-17 & Masowe oznaczenie jako przeczytane & Notification & I &
  \texttt{PATCH /notifications/read-all}: 3 nieprzeczytane &
  200; \texttt{data.updated == 3}; wszystkie \texttt{is\_read=True} w~DB \faCheck \\
\addlinespace

TC-18 & Usunięcie nieistniejącego powiadomienia & Notification & I &
  \texttt{DELETE /notifications/99999} &
  404 Not Found \faCheck \\
\addlinespace

\midrule

TC-19 & Brak pola \texttt{message} w~zapytaniu & Chat API & I &
  \texttt{POST /api/chat}: puste ciało \texttt{\{\}} &
  400; \texttt{error: "invalid\_message"} \faCheck \\
\addlinespace

TC-20 & Poprawne zapytanie do asystenta AI & Chat API & I &
  \texttt{POST /api/chat}: \texttt{\{message: "ile to 2+2?"\}} &
  200; \texttt{cached: false}; \texttt{apiUsed: true}; niepusta odpowiedź \faCheck \\
\addlinespace

TC-21 & Trafienie w~pamięć podręczną & Chat API & I &
  To samo pytanie wysłane dwukrotnie z~tego samego IP &
  Drugie wywołanie: 200; \texttt{cached: true}; brak wywołania SDK AI \faCheck \\
\addlinespace

TC-22 & Rate limiting - przekroczenie limitu & Chat API & I &
  21 żądań z~tego samego IP w~oknie 1~minuty &
  21. żądanie: 429 Too Many Requests \faCheck \\
\addlinespace

\midrule

TC-23 & Formularz kontaktowy - niepoprawny e-mail & Contact API & I &
  \texttt{POST /api/contact}: e-mail bez znaku \texttt{@} lub bez kropki &
  400 Bad Request \faCheck \\
\addlinespace

TC-24 & Formularz kontaktowy - poprawne dane & Contact API & I &
  \texttt{POST /api/contact}: name, email, message &
  200; Resend wywołany z~\texttt{to}, \texttt{subject}, \texttt{html} \faCheck \\
\addlinespace

TC-25 & Błąd Resend API & Contact API & I &
  \texttt{POST /api/contact}: Resend zwraca \texttt{ok: false} &
  500 Internal Server Error \faCheck \\
...
Scenariusze opisują wielokrokowe przepływy łączące powiązane przypadki testowe w~realistyczne sekwencje użycia systemu.

\subsubsection{ST-01: Rejestracja i aktywacja konta użytkownika}

\textbf{Cel:} weryfikacja kompletnego przepływu rejestracji od złożenia formularza do~aktywacji konta i~zalogowania.

\begin{enumerate}
  \item Użytkownik wysyła żądanie \texttt{POST /api/v1/auth/register} z~poprawnymi danymi (TC-01). \\ Oczekiwany wynik: 201, konto nieaktywne, wygenerowany kod weryfikacyjny.
  \item System wysyła e-mail z~kodem na~podany adres.
  \item Użytkownik wysyła \texttt{POST /api/v1/auth/verify-email} z~kodem z~e-maila (TC-04). \\ Oczekiwany wynik: 200, konto aktywne (\texttt{is\_active=True}).
  \item Użytkownik loguje się przez \texttt{POST /api/v1/auth/login} (TC-03). \\ Oczekiwany wynik: 200, ważny JWT w~odpowiedzi.
\end{enumerate}

\textbf{Powiązane przypadki:} TC-01, TC-03, TC-04.

\subsubsection{ST-02: Zarządzanie przestrzenią roboczą}

\textbf{Cel:} weryfikacja tworzenia przestrzeni, zapraszania członka i~usunięcia go przez ownera.

\begin{enumerate}
  \item Zalogowany użytkownik A~tworzy WS przez \texttt{POST /api/v1/workspaces} (TC-07). \\ Oczekiwany wynik: 201, rola \emph{owner}.
  \item Użytkownik A~wysyła zaproszenie do użytkownika B przez \texttt{POST .../workspaces/\{id\}/invite}.
  \item Użytkownik B akceptuje zaproszenie przez \texttt{POST .../invites/accept/\{token\}}. \\ Oczekiwany wynik: 200, rekord WorkspaceMember w~DB.
  \item Użytkownik A~usuwa użytkownika B przez \texttt{DELETE .../members/\{uid\}} (TC-09). \\ Oczekiwany wynik: 200, brak rekordu w~DB.
  \item Próba edycji WS przez użytkownika B (TC-08). \\ Oczekiwany wynik: 403 Forbidden.
\end{enumerate}

\textbf{Powiązane przypadki:} TC-07, TC-08, TC-09, TC-10.

\subsubsection{ST-03: Praca z~tablicą kanban}

\textbf{Cel:} weryfikacja pełnego cyklu życia tablicy - tworzenie, edycja, zarządzanie ulubionymi.

\begin{enumerate}
  \item Użytkownik pobiera listę tablic \texttt{GET /api/v1/boards?workspace\_id=X} (TC-11). \\ Oczekiwany wynik: 200, lista tablic przestrzeni.
  \item Użytkownik tworzy tablicę \texttt{POST /api/v1/boards} (TC-12 --- ścieżka negatywna, potem poprawna). \\ Oczekiwany wynik: 422 dla brakującej nazwy; 201 po~uzupełnieniu danych.
  \item Użytkownik zmienia nazwę tablicy przez \texttt{PUT /api/v1/boards/\{id\}} (TC-13). \\ Oczekiwany wynik: 200, zaktualizowana nazwa.
  \item Użytkownik dodaje tablicę do ulubionych \texttt{POST .../toggle-favourite} (TC-14). \\ Oczekiwany wynik: 200, \texttt{is\_favourite: true}.
\end{enumerate}

\textbf{Powiązane przypadki:} TC-11, TC-12, TC-13, TC-14.

\subsubsection{ST-04: Asystent AI z~cache'owaniem i~rate limitingiem}

\textbf{Cel:} weryfikacja walidacji wejścia, pamięci podręcznej odpowiedzi i~mechanizmu ograniczania żądań.

\begin{enumerate}
  \item Użytkownik wysyła żądanie bez pola \texttt{message} (TC-19). \\ Oczekiwany wynik: 400, \texttt{error: "invalid\_message"}.
  \item Użytkownik wysyła poprawne pytanie (TC-20). \\ Oczekiwany wynik: 200, \texttt{cached: false}, odpowiedź modelu AI.
  \item Użytkownik ponawia to samo pytanie (TC-21). \\ Oczekiwany wynik: 200, \texttt{cached: true}, brak wywołania SDK.
  \item Użytkownik wysyła 21 żądań z~tego samego adresu IP w~krótkim czasie (TC-22). \\ Oczekiwany wynik: 21. żądanie zwraca 429 Too Many Requests.
\end{enumerate}

\textbf{Powiązane przypadki:} TC-19, TC-20, TC-21, TC-22.

\subsection{Podsumowanie pokrycia testami}

\begin{center}
\begin{tabular}{lrrr}
\toprule
\textbf{Moduł} & \textbf{Testy jednostkowe} & \textbf{Testy integracyjne} & \textbf{Łącznie} \\
\midrule
Autentykacja        & $\sim$40 & $\sim$30 & $\sim$70 \\
Przestrzenie robocze & $\sim$15 & 12       & $\sim$27 \\
Tablice             & $\sim$10 & 10       & $\sim$20 \\
Powiadomienia       & $\sim$8  & 8        & $\sim$16 \\
Asystent AI (Chat)  & -      & 9        & 9        \\
Formularz kontaktowy & -     & 8        & 8        \\
Narzędzia i hooki   & $\sim$50 & -      & $\sim$50 \\
Tablica interaktywna & $\sim$15 & -     & $\sim$15 \\
\midrule
\textbf{Razem}      & $\sim$138 & $\sim$77 & $\sim$215 \\
\bottomrule
\end{tabular}
\end{center}
